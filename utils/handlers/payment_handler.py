...
def get_response_text(language, key):
    responses = {
        ...
        "invalid_transfer_details": {
            ...
            "sw": "Tafadhali toa kiasi halali na nambari ya gari.",
        },
        "invalid_recover_details": {
            "en": "Please provide a valid amount and fleet numbers.",
            "sw": "Tafadhali toa kiasi halali na nambari ya gari.",
        },
        "default": {
...
def handle_recover_request(bot_name, user_id, message, language):
    """Handle cash transfer request processing."""
    logger.debug("Processing recover request")
    ...
    if details:
        ...
        user_context[user_id]["pending_recover"] = {
            "amount": amount,
            "source_fleet": source_fleet,
            "destination_fleet": destination_fleet,
        }
        user_context[user_id]["awaiting_recover_confirmation"] = True

        response_text = get_response_text(language, "awaiting_recover_confirmation")
        logger.debug(f"Set awaiting confirmation. Response: {response_text}")
        return {bot_name: response_text}

    logger.debug("Invalid recover details, sending error response")
    return {bot_name: get_response_text(language, "invalid_recover_details")}


def handle_awaiting_recover_confirmation(bot_name, user_id, message, language):
    """Handle user confirmation for recovery."""
    # logger.debug(f"Checking for pending transfer request for user {user_id}")
    ...
    if user_id in user_context and user_context[user_id].get(
        ...
        if confirmation_msg in ["yes", "ndio"]:
            # Get transfer details
            transfer_details = user_context[user_id].get("pending_recover", {})
